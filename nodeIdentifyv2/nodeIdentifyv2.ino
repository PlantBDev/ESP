...
#include <EEPROM.h> //TODO include in main sketch?
#define NAMELOC 1 //Memory location on EEPROM where the nodes name is stored TODO reconsider if this is a good value for this //Also TODO move to MAIN
  ...
  for(int j = 0; j < 10; j++){
    ...
    for(int k = 0; k < 10; k++){
      if(nodeNameFound != 0){
        ...
        uint16_t packetIdSub = mqttClient.subscribe(topicString, 2);  //Subscribe to the different "node/countNNN" mqtt topics which contain a list of node identifiers
        #if IDENTITYDEBUG
        ...
        uint16_t packetIdUnsub = mqttClient.unsubscribe(topicString); //Unsubscribe from previous node/CountNNN topic
